...
2. **additionalTrustBundle** — from `openshift_additional_trust_bundle` if set;
   ...
   When `openshift_ingress_ca_cert` is set (vault), that PEM is appended so
   the installer also trusts the custom default-ingress signing CA.
...
| `openshift_ingress_tls_cert` | Optional PEM cert chain for default ingress (leaf first) |
| `openshift_ingress_tls_key` | Optional unencrypted PEM private key |
| `openshift_ingress_ca_cert` | Optional PEM CA: Proxy `trustedCA` post-install **and** appended to install-config `additionalTrustBundle` |

### Custom default ingress TLS (post-install)

When **all three** ingress PEM variables are non-empty, `tasks/post-install/ingress-tls.yaml`:

1. Creates ConfigMap `custom-ca` in `openshift-config` (`ca-bundle.crt`)
2. Sets `Proxy/cluster` `spec.trustedCA.name` to that ConfigMap
3. Creates TLS secret `custom-certs-default` in `openshift-ingress`
4. Sets `IngressController/default` `spec.defaultCertificate.name` to that secret

If any one is set without the others, the role fails. Omit all three to keep the Operator-generated wildcard cert.

Example vault entries (encrypt with `ansible-vault`):

```yaml
openshift_ingress_tls_cert: |
  -----BEGIN CERTIFICATE-----
  ...
  -----END CERTIFICATE-----
openshift_ingress_tls_key: |
  -----BEGIN PRIVATE KEY-----
  ...
  -----END PRIVATE KEY-----
openshift_ingress_ca_cert: |
  -----BEGIN CERTIFICATE-----
  ...
  -----END CERTIFICATE-----
```
